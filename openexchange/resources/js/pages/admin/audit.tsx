...
type Props = {
    logs: Log[];
    actions: string[];
    filters: { action: string; client_id: number | null };
};
...
const tone = (action: string): 'warning' | 'danger' | 'info' | 'neutral' => {
    ...
    if (
        action.startsWith('balance') ||
        action.startsWith('client.delete') ||
        action.startsWith('charge')
    ) {
        return 'danger';
    }

    if (
        action.startsWith('rate') ||
...
export default function Audit({ logs, actions, filters }: Props) {
    const setFilter = (action: string) =>
        router.get('/admin/audit', action ? { action } : {}, {
            preserveScroll: true,
            preserveState: true,
        });
    ...
    return (
        <AdminLayout
            active="audit"
            title="Audit log"
            subtitle="Every privileged action, most recent first. Append-only."
        >
            ...
            >
                <Chip on={!filters.action} onClick={() => setFilter('')}>
                    All
                </Chip>
                {actions.map((a) => (
                    <Chip
    ...
    return (
        <button
            ...
            style={{
                ...
                border:
                    '1px solid ' + (on ? 'transparent' : 'var(--ox-border)'),
