...
import { Card, StatCard, Button, Icon, LineArea, Donut, RankBars } from '@/components/oe';
...
export default function Usage({ stats, dailySpend = [0], byProvider = [], bySource = [], table = [] }: Props) {
    ...
    return (
        <ConsoleLayout active="usage" title="Usage" subtitle="Current billing period"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <Card padding="none">
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--ox-divider)', fontSize: 'var(--ox-text-lg)', fontWeight: 600 }}>Breakdown by provider</div>
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 520 }}>
                            <thead><tr>{['Provider', 'Requests', 'Tokens', 'Spend'].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
                            <tbody>
                                {table.length === 0 && <tr><td style={{ ...td, color: 'var(--ox-text-subtle)' }} colSpan={4}>No usage metered yet.</td></tr>}
                                {table.map((r, i) => (
                                    <tr key={i}>
                                        <td style={{ ...td, color: 'var(--ox-text)', fontWeight: 600, fontFamily: 'var(--ox-font-sans)' }}>{r[0]}</td>
                                        <td style={td}>{r[1]}</td><td style={td}>{r[2]}</td>
                                        <td style={{ ...td, color: 'var(--ox-text)', fontWeight: 600 }}>{r[3]}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
