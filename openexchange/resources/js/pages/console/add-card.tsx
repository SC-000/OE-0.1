...
export default function AddCard({
    publishableKey,
    customerId,
    widgetBase = 'https://billings.systems',
    hasToken = false,
    topupAmount = 10,
}: Props) {
    const configured = !!(publishableKey && customerId);
    const testReason =
        hasToken && !publishableKey
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const w = window as unknown as {
                SetupWidget?: (o: unknown) => void;
            };

            if (cancelled || !w.SetupWidget || !widgetRef.current) {
                return;
            }
            ...
            w.SetupWidget({
                config: {
                    ...
                    showLogo: false,
                    title: '',
                    buttonText: 'Connect card securely',
                    colors: {
                        primary: '#33c13e',
                        background: '#fffdf8',
                        text: '#122023',
                    ...
                    onSuccess: (data: {
                        ...
                        pmRef.current = data?.payment_method ?? null;
                        setStep('connecting');
                        setStage(0);
                    },
    ...
    useEffect(() => {
        if (step !== 'done' || persisted.current) {
            return;
        }
        ...
        const [em, ey] = (card.exp || '')
            .split('/')
            .map((s) => parseInt(s.trim(), 10) || 0);
        router.post(
            '/console/billing/card',
            {
                payment_method_id:
                    (pm.id as string) ||
                    'pm_test_' + Math.random().toString(16).slice(2, 10),
                brand: (pm.type as string) || (pm.brand as string) || 'visa',
                last4: (pm.last4 as string) || digits.slice(-4) || '4242',
                exp_month:
                    (pm.expiry_month as number) ||
                    (em >= 1 && em <= 12 ? em : 8),
                exp_year:
                    (pm.expiry_year as number) ||
                    (ey ? (ey < 100 ? 2000 + ey : ey) : 2028),
            },
    ...
    return (
        <ConsoleLayout
            ...
            subtitle="Securely connect a card for prepaid top-ups"
        >
                ...
                {step === 'form' && (
                    <>
                        ...
                        >
                            ...
                            <PaymentCard
                                number={card.number ? fmtNum(card.number) : ''}
                                exp={card.exp}
                                holder={card.name}
                            />
                                    ...
                                    <>
                                        ...
                                        >
                                            <Icon
                                                ...
                                                color="var(--ox-text-subtle)"
                                            />
                                            Test mode — no live charge; the card
                                            is saved so you can test.{' '}
                                            {testReason}
                                        </span>
                                    </>
                                )}
                        ...
                        >
                            Adding your card places a one-time{' '}
                            <span
                                className="ox-mono"
                                                ...
                                                {done ? (
                                                    ...
                                                    <span
                                                        style={{
                                                            ...
                                                            height: 10,
                                                            borderRadius: '50%',
                                                            border: '2px solid var(--ox-border-strong)',
                                                        }}
                                                    />
                                                )}
                                            </span>
                    ...
                    >
                        <span
                            ...
                            style={{
                                ...
                                height: 72,
                                borderRadius: '50%',
                                background: 'var(--ox-primary-subtle)',
                                marginBottom: 8,
                            }}
                        >
                            <Icon
                        ...
                        >
                            Your card is linked and auto top-up is enabled —
                            your account keeps running without interruption.
                        </p>
                        <Button
                            size="lg"
                            style={{ marginTop: 20 }}
                            onClick={() => router.visit('/console/billing')}
                            trailingIcon={
                                <Icon
                                    name="arrow-right"
                                    size={16}
...
const nodeLbl: React.CSSProperties = {
